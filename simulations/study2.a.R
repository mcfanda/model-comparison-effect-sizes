...
one_model<-function(r2, N,  k, model, rho=.0) {
      ...
      beta<-make_beta(model,k) # see functions.R
      xcov<-make_xcov(k,rho)   # see functions.R
...
find_n <- function(r2, k, model, rho = 0,search_rep=10) {
  found <- FALSE
  Nt  <- round(gzlmpower::power.lrt(r2, df = model_df(model), power = .80, prob = prob_for_model(model))$N) 
  ...
  cpower<-.80
  while (!found) {
    trials <- future.apply::future_lapply(1:search_rep, function(i) one_model(r2, k = k, N = N, model=model, rho=rho), future.seed = TRUE)
    ...
    if (power>cpower) {
      ...
      confirm <- future.apply::future_lapply(1:(search_rep*5), function(i) one_model(r2, k = k, N = N, model=model, rho=rho), future.seed = TRUE)
...
runner$design<-list(r2=seq(.025,.2,.025),model=c("logistic","multinomial","ordinal"))

results<-runner$experiment(Rep=1)

save(results,file=here::here("simulations/Data/study2.a.data.Rdata"))
